...
//   useEffect(() => {
//     if (!isSolving) return; // Exit if visualization is not in progress
...
//       setIsSolving(false);
//       if (steps.length > 0) {
//         setBoard(steps[steps.length - 1]); // Show the final solution
//       }
//     }
...
//   const resetBoard = () => {
//     setIsSolving(false); // Stops visualization
//     setBoard(Array(N).fill().map(() => Array(N).fill(false))); // Resets the board
//     setSolutions([]); // Clears solutions
//     setSteps([]); // Clears animation steps
//     setCurrentStep(0); // Resets step counter
//     setCurrentSolution(0); // Resets solution index
//   };
...
//       <div
//         className="grid gap-1 bg-gray-400 p-2 rounded-lg"
//         style={{
//           gridTemplateColumns: `repeat(${N}, 1fr)`,
//           maxWidth: `calc(100vmin - 2rem)`,
//           width: `calc(${N} * 3rem)`,
//         }}
//       >
//         {board.map((row, i) =>
...
const NQueensVisualizer = () => {
  ...
  const goToNextSolution = () => {
    if (currentSolution < solutions.length - 1) {
      ...
      setBoard(solutions[nextSolution]);
    }
  };

  const goToPreviousSolution = () => {
    if (currentSolution > 0) {
      const previousSolution = currentSolution - 1;
      setCurrentSolution(previousSolution);
      setBoard(solutions[previousSolution]);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      ...
      <div
        className="grid gap-1 bg-gray-400 p-2 rounded-lg border border-black"
        style={{
          gridTemplateColumns: `repeat(${N}, 1fr)`,
          maxWidth: `calc(100vmin - 2rem)`,
          width: `calc(${N} * 3rem)`
      ...
      {solutions.length > 0 && (
        <div className="mt-4 text-green-600 font-medium">
          {`Total Solutions: ${solutions.length} | Showing Solution ${currentSolution + 1} of ${solutions.length}`}
        </div>
      )}
    </div>
  );
